...
> 1. `/genres/[g]`'s `generateStaticParams` self-fetches `/api/genres` at build time with no live server to hit yet — unlike `/artist/[slug]`'s equivalent, it had no try/catch fallback, so the failed fetch threw and crashed the entire Vercel build (`Failed to collect page data`). Fixed by matching the existing try/catch → `[]` pattern.
> 2. `/genres` (no dynamic segment, so no `generateStaticParams` to guard) hit the same build-time self-fetch wall with nowhere to catch it — a plain static page unconditionally prerendered at build. Fixed by marking it `export const dynamic = 'force-dynamic'` so it renders at request time instead, when a real server exists.
> 3. **The big one:** every page that self-fetches its own `/api/*` routes (`getBaseUrl()` in `web/src/lib/api.ts`) 500'd in production, but the routes worked fine when curled directly. Cause: Vercel's Deployment Protection blocks unauthenticated requests to every `*.vercel.app` hostname by default (custom domains are exempt) — `VERCEL_URL` always resolves to a protected hostname, so the internal fetch got redirected to Vercel's login page and `JSON.parse()` on that HTML threw. Fixed by adding a `SITE_URL` env var (`https://music.deanslist.dev`) that `getBaseUrl()` now prefers over `VERCEL_URL`.
> 4. Leaderboard's `1M+` size band silently fell back to "all sizes" — `href={...&slice_key=${sk}}` built the URL by hand, and an unencoded `+` gets decoded back to a space by `URLSearchParams` (form-urlencoded convention), so `slice_key` arrived server-side as `"1M "` and failed validation. The same unencoded pattern existed independently in `api.ts`'s `getLeaderboard()`, which builds the page's own internal self-fetch URL — so even a correctly-encoded incoming URL would have broken a second time on that hop. Fixed both call sites with `encodeURIComponent`, matching the pattern `search()` already used.
>
> **Investigated, not a bug:** the `500k-1M` leaderboard band appearing to contain artists over 1M latest listeners. `size_band` buckets by **starting** listeners (13 weeks ago), not current — confirmed against the DB (e.g. JAŸ-Z started at 816k, correctly bucketed `500k-1M`, now at 1.17M). Since Last.fm listener counts are cumulative and monotonically increasing, an artist can cross a band boundary during the tracking window while staying correctly bucketed by where they started — this is the same starting-listener methodology the published growth-by-quintile findings use; bucketing by current listeners instead would contradict those numbers.
>
> **`notFound()` re-verified against the live Vercel deployment (2026-08-07) — the 200-status quirk is confirmed, and a second, worse bug was found alongside it:**
> 1. **Quirk confirmed live, not fixed by Vercel's edge:** `curl`ing `/artist/999999999-doesnotexist` (well-formed slug, no matching artist) returns the correct not-found UI but with HTTP 200, exactly matching the self-hosted `next start` behavior from Stage D. The hoped-for edge correction doesn't happen. `robots: {index: false}` on `not-found.tsx` remains the mitigation; this is an upstream Next.js App Router limitation, not something fixable in this codebase.
> 2. **New bug, now fixed:** a *malformed* slug (no leading digits — e.g. a stray link or a crawler probing garbage paths) made `/api/artists/[slug]` 400 instead of 404 (`extractArtistIdFromSlug` returns `null` before any DB lookup happens). `artist/[slug]/page.tsx`'s catch block only matched `err.status === 404` and rethrew everything else, so the 400 became an uncaught exception — the page crashed to Next's generic unstyled 500 error page (confirmed live: `curl /artist/this-artist-does-not-exist-xyz123` → HTTP 500, `x-next-error-status: 500`, Pages-router-style `_error` HTML, not the app's `not-found.tsx`). Fixed by widening the catch to treat both 400 and 404 as "render not-found" (`web/src/app/artist/[slug]/page.tsx:66`). Verified locally via `next build` + `next start`: the malformed-slug case now returns 200 with the friendly not-found page instead of a raw 500.
>
> The reciprocal project card on the Astro apex site (`deanslist.dev` → `music.deanslist.dev`) is still open — that's a change in a different repo.
>
...
- Env vars, all server-only (no `NEXT_PUBLIC_`): `DATABASE_URL_READONLY`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `REVALIDATE_SECRET`, `IP_HASH_SALT`, `SITE_URL` (`https://music.deanslist.dev`).
- **Found live 2026-08-07:** Vercel's Deployment Protection blocks unauthenticated requests to every `*.vercel.app` hostname by default (custom domains are exempt). Pages that self-fetch their own `/api/*` routes (see the architecture note in `web/src/lib/api.ts`) were doing so via `VERCEL_URL`, which always points to a protected hostname — every such page 500'd in production (the internal fetch got redirected to Vercel's login page; `JSON.parse()` on that HTML threw). Fixed by adding `SITE_URL` and having `getBaseUrl()` prefer it over `VERCEL_URL`.
