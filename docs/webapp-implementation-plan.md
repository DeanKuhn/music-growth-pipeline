...
>
> **Still open from Stage E's original scope:** the `notFound()` 200-status quirk (flagged during Stage D) hasn't been re-verified against the live Vercel deployment yet. The reciprocal project card on the Astro apex site (`deanslist.dev` → `music.deanslist.dev`) hasn't been added — that's a change in a different repo.
>
