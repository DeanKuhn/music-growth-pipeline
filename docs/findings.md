...
8. **Resolved (2026-09-23, `8aaadf0`).** Last.fm hosts a mojibake duplicate of 照井順政 (Junsei Terui): artist `2552` is named `ç…§äº•é †æ”¿` — the UTF-8 bytes `E7 85 A7 / E4 BA 95 / E9 A0 86 / E6 94 BF` decoded as Windows-1252. It ranked #19 on /leaderboards (Fastest Growing %, All sizes, snapshot 2026-09-13).

   **Cause is Last.fm, not the pipeline.** `artist.getInfo` (`autocorrect=0`, 2026-09-23) returns the garbled name byte-for-byte identical to our stored value (`c3a7e280a6c2a7c3a4c2bae280a2c3a920e280a0c3a6e2809dc2bf`), no mbid, 64,945 listeners, URL slug `…%C3%A9+%E2%80%A0…`. Postgres is UTF8 end to end (server, client, database). The correctly named artist exists separately as `67166` (mbid `ae0e6222-7e5a-4c53-84ff-20f0626d39bf`), so Last.fm treats them as two artists, each with its own scrobbles: the garbled page collects plays from clients sending mis-decoded tags. Listener counts are independent (2026-09-20: 64,662 vs 319,662) and overlap by an unknown amount, so the two cannot be summed.

   **Not reversible.** Last.fm stored the `A0` byte (NBSP, the middle byte of 順) as a plain space (`20`), so re-encoding as Windows-1252 and decoding as UTF-8 yields invalid UTF-8. A generic repair would also fail on UTF-8 bytes `81/8D/8F/90/9D`, which Windows-1252 leaves undefined.

   **Scope: one artist.** A regex scan of `artists` for Windows-1252 renderings of UTF-8 lead + continuation bytes matched 2 of 22k+: this one and `Tomáš Dvořák` (`11468`, false positive — real name with an mbid).

   **Why it matters beyond display:** the duplicate grew +1.24% the week ending 2026-09-13 against +0.27% for the real page — a small, split-off page reads as a growth signal it isn't.

   **Decision:** exclude via a dbt seed (`excluded_artists`: artist_id, reason) filtered in `api_leaderboard`; raw data untouched. No reverse-decoding repair. `weekly_snapshot.sh` now runs `dbt seed` before `dbt run`. Verified: `dbt build --select excluded_artists api_leaderboard` passes and 2552 no longer appears in `api_leaderboard`.
