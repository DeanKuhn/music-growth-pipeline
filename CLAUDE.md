...
## Tech Stack
Python ingestion → Postgres (Neon) → dbt (dbt-postgres) → SQL analyses. Source: Last.fm API (key auth, read-only). Automation: GitHub Actions weekly cron. Power BI report on a live Postgres connection. Web app: Next.js on EC2 (Ubuntu) behind Caddy reverse proxy, systemd-managed.
...
| `web/vercel.json` | Vestigial from the Vercel deployment — `ignoreCommand` skipped Vercel rebuilds for non-`web/` changes. Harmless to keep but no longer functional. |
| `deploy.sh` | One-command deploy: SSHes into EC2, pulls, builds, restarts the systemd service. No secrets — SSH key path and host are env-var overridable (`DEPLOY_KEY`, `DEPLOY_HOST`). |
