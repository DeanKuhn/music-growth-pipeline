...
## Tech Stack
Python ingestion → Postgres (self-hosted on Hetzner) → dbt (dbt-postgres) → SQL analyses. Source: Last.fm API (key auth, read-only). Automation: cron job on server (`weekly_snapshot.sh`). Power BI report on a live Postgres connection. Web app: Next.js on same Hetzner instance behind Caddy reverse proxy, systemd-managed.
...
**Now:** Stages A, C, D, and E are **complete**. Stage A: all 7 `dbt/models/api/` models built and tested, `app_readonly` grants verified against the live DB after a rebuild (2026-08-05) — plus an 8th, `api_genres`, added during Stage C. Stage C: `web/` (Next.js 15 App Router, TS) built, all 8 endpoints verified end-to-end against the live DB with the real `app_readonly` role (2026-08-05) — see `docs/webapp-implementation-plan.md`'s PROGRESS block for the two validation bugs and one credential-mixup incident found and fixed during that verification. Stage D: all routes built (landing, artist page with charts/comparisons, leaderboards, genres, how-it-works) and verified via a real `next build` + `next start` against the live DB (2026-08-05) — see the plan's PROGRESS block for a Stage-C-era type error and a Postgres-numeric-as-string bug both caught here, a forced React 18→19 bump, and a known Next.js `notFound()` status-code quirk still to re-check on the live deployment. Stage E: originally deployed to Vercel (2026-08-07), migrated to a self-hosted EC2 instance behind Caddy (2026-08-25) to avoid Vercel free tier limits, then migrated again to a Hetzner Cloud instance (2026-09-21) with self-hosted Postgres replacing Neon (which hit its storage limit). The Hetzner migration swapped `@neondatabase/serverless` for `postgres` (postgres.js), removed Edge runtime from API routes, and replaced the GitHub Actions weekly cron with a local `weekly_snapshot.sh` + crontab. Pages query the DB directly via `queries.ts`; API routes remain as thin wrappers for client-side consumers. Upstash-backed rate limiting still wired in. Deploy via `./deploy.sh`. Remaining: reciprocal project card on the Astro apex site (different repo). Next up is Stage B: ingestion scale-up.
...
| `web/src/lib/db.ts` | Exports `sql` (a `postgres()` tagged-template client from postgres.js, replacing the former Neon serverless driver) and `assertReadonlyRole()` — the latter queries `current_user` once per warm instance and throws if it isn't `app_readonly`, catching a wrong-but-different `DATABASE_URL_READONLY` that the byte-equality check above it can't. Called from `rate-limit.ts`, not here directly. |
...
| `web/src/lib/queries.ts` | Direct DB query functions used by both `api.ts` (server components) and the API route handlers. Single source of truth for all SQL — neither pages nor routes inline their own queries. |
| `web/src/lib/api.ts` | Server-only. Exports typed functions (`getArtistProfile`, `getLeaderboard`, etc.) that call `queries.ts` directly and apply `coerceNumericStrings`. Pages import from here. **Does not self-fetch** — the old `getBaseUrl()`/`getJson()` HTTP loop was removed during the EC2 migration. |
| `web/src/lib/api-types.ts` | Client-safe. Types, `ApiError`, and `coerceNumericStrings` — everything client components need from the API layer without pulling in `db.ts`. Client components must import from here, not `api.ts`, to avoid bundling the Neon driver into the browser. |
...
| `web/vercel.json` | Vestigial from the Vercel deployment. Harmless to keep but no longer functional. |
| `deploy.sh` | One-command deploy: SSHes into Hetzner (`bibba@music.deanslist.dev`), pulls, builds, restarts the systemd service. Host overridable via `DEPLOY_HOST`. |
| `weekly_snapshot.sh` | Replaces the former GitHub Actions workflow. Runs snapshot → dbt → generate_stats → git push → restart web. Called by crontab (Sundays 9am UTC). Logs to `logs/weekly_snapshot.log`. |
...
| `.github/workflows/weekly_snapshot.yml` | **Legacy** — formerly ran the weekly pipeline via GitHub Actions. Replaced by `weekly_snapshot.sh` + crontab on the Hetzner server (2026-09-21). Kept for reference. |
