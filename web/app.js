...
function renderSearchResults(matches) {
	...
	box.innerHTML = matches.slice(0, 8).map(function(a) {
		...
		class="flex justify-between gap-4 px-4 py-2 hover:bg-gray-800 text-gray-100">${a.display_name}
		<span class="text-gray-500 text-sm whitespace-nowrap">${formatNum(a.latest_listeners)}
		listeners</span></a>`;
	}).join("");
...
async function initStats() {
	...
	stats.innerHTML = `
		...
		<div class="border border-gray-800 rounded-lg p-4 text-center">
			<div class="text-3xl font-bold">${health.weeks_in_series}</div>
			<div class="text-sm text-gray-400 mt-1">Weeks of data</div>
		</div>
		<div class="border border-gray-800 rounded-lg p-4 text-center">
			<div class="text-3xl font-bold">${health.genres_total}</div>
